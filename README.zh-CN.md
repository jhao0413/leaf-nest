...
对于 Docker Compose 部署，`app` 容器会把可选的 `SELF_HOST_*` 变量映射成运行时的 `APP_URL`、`DATABASE_URL`、`BETTER_AUTH_URL` 与 S3 endpoint。这样本地开发默认值可以保留，而远程 self-host 时也能切到公开域名和浏览器可访问的对象存储地址。
