...
export default function BookNotesPage() {
  ...
  const buildShareImageBlob = useCallback(async (item: HighlightWithBook) => {
    ...
    const drawWrappedText = (
      ...
      const fontFamily = family || '"LXGW WenKai", "Noto Serif SC", "PingFang SC", "Microsoft YaHei", serif';
      ctx.fillStyle = color;
      ...
      if (maxLines && lines.length > maxLines) {
        ...
        if (last && !last.endsWith('…')) {
          ctx.fillText(`${last.slice(0, Math.max(0, last.length - 2))}…`, startX, currentY - lineHeight);
        }
      }
  ...
  const copyShareImageToClipboard = useCallback(async (imageBlob: Blob) => {
    ...
    await navigator.clipboard.write([new ItemCtor({ 'image/png': imageBlob } as Record<string, Blob>)]);
  }, []);
            ...
            >
              <div className="flex items-center justify-center h-6">
                {copyMessage ? (
                  ...
                  <div className={`flex items-center gap-2 text-[15px] font-semibold text-gray-900 dark:text-gray-100 transition-opacity ${(isCopying || isSharePreviewLoading) ? 'opacity-50' : 'group-hover:text-blue-600 dark:group-hover:text-blue-400'}`}>
                    <Copy size={18} className="text-gray-400 dark:text-gray-500 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors" />
                    <span>{isCopying ? t('shareCopying') : isSharePreviewLoading ? t('loading') : t('shareCopy')}</span>
                  </div>
                )}
              </div>
            </button>
