...
export default function NotesPage() {
  ...
  const loadHighlights = useCallback(async () => {
    ...
    const data = await highlightsRepository.listAll();
    ...
    for (const h of data) {
      if (!groupMap.has(h.bookId)) {
        groupMap.set(h.bookId, {
          bookId: h.bookId,
          bookName: h.book.name || 'Unknown',
          bookCoverUrl: h.book.coverUrl,
          noteCount: 0,
          currentChapter: h.book.currentChapter,
          percentage: h.book.percentage
        });
      }
  ...
  useEffect(() => {
    ...
    void loadHighlights();
  }, [loadHighlights, sessionStatus]);

  const formatProgress = (book: GroupedHighlights) => {
    const percentValue =
      typeof book.percentage === 'number' ? book.percentage : Number(book.percentage);
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <h2
                      className="font-bold text-base sm:text-lg font-lxgw leading-snug truncate"
                      title={book.bookName}
                    >
                      {book.bookName}
                    </h2>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                      {t('noteCount', { count: book.noteCount })}
                    </p>
                  </div>
