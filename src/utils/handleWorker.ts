  ...
  try {
    db.exec({
      sql: `
        ...
        WHERE id = ?;
      `,
      bind: [data.currentChapter, data.currentPage, data.textAnchor, data.percentage, data.bookId]
    });
    console.log(
      `Reading progress updated for book ${data.bookId}: chapter ${data.currentChapter}, page ${data.currentPage}, ${data.percentage}%`
    );
