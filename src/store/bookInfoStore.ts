...
export type BookInfoListType = {
  bookInfoList: BookBasicInfoType[];
  setBookInfoList: (info: BookBasicInfoType[]) => void;
};

export const useBookInfoListStore = create<BookInfoListType>((set) => ({
  bookInfoList: [],
  setBookInfoList: (info: BookBasicInfoType[]) => set({ bookInfoList: info }),
}));
